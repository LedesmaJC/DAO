...
def enviar_prestamo():
    ...
    f_prestamo = entry_f_prestamo.get()
    f_devolucion_estimada = entry_f_devolucion_estimada.get()
...
def limpiar_campos():
    ...
    combobox_libros.set('')
    entry_f_prestamo.delete(0, tk.END)
...
def iniciar_ventana_prestamo():
    global combobox_usuarios, combobox_libros, entry_f_prestamo, entry_f_devolucion_estimada, entry_f_devolucion_real, entry_observacion, lbl_validacion, ventana
    ...
    # Fecha de préstamo
    tk.Label(frame, text="Fecha de Préstamo:", font=fuente_label, bg="#FFFFFF", fg="#666666").grid(row=3, column=0, sticky="e", padx=10, pady=8)
    entry_f_prestamo = tk.Entry(frame, font=fuente_entry, width=25, bd=2, relief="groove")
    entry_f_prestamo.grid(row=3, column=1, pady=8)

    # Fecha de devolución estimada
    tk.Label(frame, text="Fecha Devolución Estimada:", font=fuente_label, bg="#FFFFFF", fg="#666666").grid(row=4, column=0, sticky="e", padx=10, pady=8)
    entry_f_devolucion_estimada = tk.Entry(frame, font=fuente_entry, width=25, bd=2, relief="groove")
    entry_f_devolucion_estimada.grid(row=4, column=1, pady=8)
